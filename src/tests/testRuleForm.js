...
async function testRuleForm() {
    ...
    try {
        ...
        assert.strictEqual(await successAlert.getText(), 'Rregullat u shtuan me sukses!');

        // Prit për shkarkimin e faqes së AdminDashboard pasi të kthehet nga formulari
        await driver.wait(until.urlIs('http://localhost:8080/#/AdminDashboard'), 5000);

        console.log('Formulari u testua me sukses!');
    } catch (error) {
